...
namespace CLKsFATXLib.Streams
{
    ...
    public class Reader : System.IO.BinaryReader
    {
        ...
        public string ReadHexString(int length, bool BigEndian)
        {
            ...
            if (BigEndian)
            {
                ...
                {
                    ch[0] = (char)Buffer[x];
                    text.AppendFormat("{0,0:X2}", (int)ch[0]);
        ...
        public override int Read(byte[] array, int offset, int count)
        {
            ...
            else
            {
                ...
                if (((count - v_Cluster) + v_bToRemove - v_eToReadNotRounded) != 0)
                {
                    ...
                    for (int i = 0; i < (ClusterSpanned(v_ToReadBetween)-1); i++)    
                    {
                        ...
                        //Array.Copy(array, DataRead, b_Return,0 , b_Return.Length);
